...
def send_data(agent, pdu):
    ...
    if len(str.encode(pdu)) > BUFFER_SIZE:
        return "Data length exceed the maximum allowed!"
...
if __name__ == '__main__':
    ...
    args = sys.argv[1:]
    if len(args) < 4:
        print "This program takes exactly 4 arguments, {} given. Usage example:".format(len(args))
        print "python manager_get.py {community_string} {IP_Address} {port} {oid}"
        sys.exit(1)
    else:
        print run_get(args)
        sys.exit(0)
